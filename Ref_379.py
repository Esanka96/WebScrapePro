...
while url_index < len(url_list):
    try:
        ...
        currentSoup=get_soup(get_soup(url).find("a",string=lambda text:text and "Current Issue" in text)["href"])

        try:
            monthYear=currentSoup.find("span",class_="current-issuetitle-toc").h5.get_text(strip=True)
            Month,Year=re.search(r"(\w+), (\d{4})",monthYear).groups()
        except:
            Month,Year="",""

        try:
            issueVolume = re.sub(r'\s+',' ',currentSoup.find("div", class_="current-issuenumber-toc").get_text(strip=True))
            Volume,Issue=re.search(r"No. (\d+) \((\d+)\)",issueVolume).groups()
        except:
            Issue,Volume="",""
        Day=""

        All_articles=currentSoup.findAll("div",class_="one-article-intoc")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = Art_index.a["href"].lstrip()
                ...
                try:
                    DOI=Article_details.find("span",class_="galley-doi-value").a.get_text(strip=True).split("org/")[-1]
                except:
                    DOI=""

                try:
                    prePagerange=Article_details.find("div",class_="article-fulltext").findAll("p")[2].get_text(strip=True)
                    Page_range=re.search(r'\):(.*?). doi', prePagerange).group(1)
                except:
                    Page_range=""

                error_variable = "pdf link"
                pdf_link=Article_details.find("a",class_="galley-link obj_galley_link pdf")["href"]
