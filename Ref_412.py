...
while url_index < len(url_list):
    try:
        ...
        url="https://zniso.fcgie.ru/jour/user/setLocale/en_US?source=%2Fjour"
        ...
        languageList =[currentSoup,
                       get_soup("https://zniso.fcgie.ru/jour")]


        try:
            monthYear=currentSoup.find("div",class_="tocArticle flex").find("div",class_="title").get_text(strip=True)
            Volume,Issue,Year=re.search(r"Vol (\d+), No (\d+) \((\d{4})\)",monthYear).groups()
        except:
            Volume,Issue,Year="","",""
        Month,Day="",""


        All_articles=currentSoup.findAll("div",class_="issueArticle flex")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = Art_index.find("div",class_="title").a["href"]

                error_variable = "article title"
                Article_title=Art_index.find("div",class_="title").get_text(strip=True)

                try:
                    Page_range = Art_index.find("span",class_="pages grey").get_text(strip=True)
                except:
                    Page_range=""

                try:
                    DOI=get_soup(Article_link).find("a",attrs={"id":"pub-id::doi"}).get_text(strip=True).rsplit("org/",1)[-1]
                except:
                    DOI=""

                error_variable = "pdf link"
                pdf_link=Art_index.find("a",string=lambda text:text and "PDF" in text)["href"].replace("view","download")
