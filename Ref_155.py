...
while url_index < len(url_list):
    try:
        ...
        currentLink="https://www.ams.org"+get_soup(url).find("ul",class_="recipients").find("li",class_="h4").find_next_sibling("li",
                            class_="h4").find_previous_sibling("li").a["href"]
        baseUrl=currentLink.split("home")[0]
        currentSoup=get_soup(currentLink)

        try:
            issueVolume = currentSoup.find("article",class_="contentList").find("h2").text.strip()
            pattern = r"Volume (\d+), Number (\d+)"

            match = re.search(pattern, issueVolume)
        ...
        All_articles=currentSoup.findAll("dl")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_title=All_articles[article_index].find("dt").a.text.strip()
                Article_link=baseUrl+All_articles[article_index].find("dt").a["href"]
                try:
                    pageRangeYear = All_articles[article_index].findAll("dd")[1].text.strip().split()
                    Page_range=pageRangeYear[-1]
                    Year=yearCheck(re.sub(r'[^0-9]+','',pageRangeYear[-2]))
                except:
                    Page_range,Year = "",""
                ...
                try:
                    DOI = All_articles[article_index].findAll("dd")[2].text.strip().rsplit("org/",1)[-1]
                except:
                    DOI=""

                pdf_link=baseUrl+All_articles[article_index].find("dd",class_="mathviewerPdfLink").findAll("a")[-1]["href"]
