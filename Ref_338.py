...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        baseUrl="https://psyjournals.ru"

        currentSoup=get_soup(baseUrl+get_soup(url).find("main",attrs={"id":"maincontent"}).find("section").find("ul").findAll("li")[-1].a["href"])

        try:
            preMonth=currentSoup.find("h2",string=lambda text:text and "Issue Contents" in text).get_text(strip=True)
            Year,Volume,Issue=re.search(r"Issue Contents (\d{4}). Vol. (\d+), no. (\d+)",preMonth).groups()
        except:
            Year,Volume,Issue="","",""
        Month,Day="",""
        print(Year,Volume,Issue)

        All_articles = currentSoup.find("div", class_="journalissue-toc publications-list").findAll("article",class_="publications-list-item")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseUrl+Art_index.find("div",class_="publications-list-item-body-title").a["href"]

                error_variable = "article title"
                Article_title=Art_index.find("h5",class_="publications-list-item-body-title-header").get_text(strip=True)

                try:
                    Page_range=Art_index.find("div",class_="publications-list-item-body-pages").get_text(strip=True).split("p.")[-1].strip()
                except:
                    Page_range=""

                try:
                    DOI=Art_index.find("a",href=lambda href:href and "org/10." in href).get_text(strip=True)
                except:
                    DOI=""

                error_variable = "pdf link"
                pdf_link=baseUrl+Art_index.find("a",string=lambda href:href and "PDF" in href)["href"]
