...
while url_index < len(url_list):
    try:
        ...
        currentSoup=get_soup(get_soup(url).find("ul",class_="pkp_navigation_primary pkp_nav_list").findAll("ul")[2].find("a")["href"])

        arbLink=currentSoup.find("li",class_="locale_ar_IQ").a["href"]
        arbContent=get_soup(arbLink)

        languageList =[currentSoup,arbContent]

        All_articles=[j for i in currentSoup.find("div",class_="sections").findAll("div",class_="section") for j in i.findAll("div",class_="obj_article_summary")]
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                try:
                    volumeYear=Article_details.find("div",class_="page page_article").findAll("a")[2].text.strip().split()
                    ...
                    try:
                        ...
                        Volume = ""
                    try:
                        Issue = re.sub(r'[^0-9]+','',volumeYear[3])
                    except:
                        print("Failed to find issue")
                        Issue = ""
                    try:
                        Year = yearCheck(re.sub(r'[^0-9]+','',volumeYear[4]))
                    except:
                ...
                try:
                    DOI=re.sub(r'\s+'," ",Article_details.find("section",class_="item doi").find("a").text.strip()).rsplit("org/",1)[-1]
                except:
                    print("Failed to find doi")
                    DOI = ""

                pdf_link=get_soup(Article_details.find("a",class_="obj_galley_link pdf").get("href")).find("a",class_="download")["href"]
