...
while url_index < len(url_list):
    try:
        ...
        currentSoup=get_soup("https://www.nomos-elibrary.de"+get_soup(url).find("div",
                    class_="pl-journals__wrapper").find("div").a["href"])

        try:
            issueVolume = re.sub(r'\s+'," ",currentSoup.find("div",class_="pl-card__tags").findAll("div",class_="pl-tag__row")[-1].text.strip())
        ...
        All_articles=currentSoup.find("div",{"id":"panel-content"}).findAll("li",attrs={"data-doi":True})
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_title=All_articles[article_index].find("a",class_="pl-contents__link")

                try:
                    Page_range=Article_title.find("span",class_="pl-contents__pages").text.strip()
                except:
                    Page_range=""

                if Article_title.find("span",class_="pl-contents__pages"):
                    Article_title.find("span",class_="pl-contents__pages").extract()

                if Article_title.find("span",class_="pl-contents__authors"):
                    Article_title.find("span",class_="pl-contents__authors").extract()

                Article_title=Article_title.text.strip()

                Article_link="https://www.nomos-elibrary.de"+All_articles[article_index].find("a",class_="pl-contents__link")["href"]

                try:
                    DOI=All_articles[article_index].get("data-doi")
                except:
                    DOI=""
                Month,Day="",""

                pdf_link=f"https://www.nomos-elibrary.de/{DOI}.pdf?download_chapter_pdf=1&page=1"
