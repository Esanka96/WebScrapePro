...
def getPdfWithProxies(url):
    #pdf_content = requests.get(pdf_link, proxies=get_random_proxy(), headers=headersChines).content
    #pdf_content = requests.get(pdf_link, headers=headersChines).content
    #return pdf_content
...
def get_soup_CN(url):
    response = requests.get(url,headers=headersChines,stream=True)
...
while url_index < len(url_list):
    try:
        ...
        url="https://www.researching.cn/en/journalmain/gxxb"
        ...
        preEngLink=get_soup("https://www.researching.cn"+get_soup(url).find("div",class_="management_List area").findAll("div",
                    class_="management_List1")[-1].find("a")["href"])
        currentLink=first_div_without_span = next((div for div in preEngLink.find_all('div', class_='contain_left_list_img_box') if not div.find('span')),None)
        engLink="https://www.researching.cn" +currentLink.find("a")["href"]

        currentSoup=get_soup(engLink)

        try:
            volumeYear=Year_volume=currentSoup.find("div",class_="textBox").findAll("div")

            try:
                Volume = volumeYear[1].text.strip().split()[1]
            except:
                print("Failed to find volume")
                Volume = ""
            try:
                Issue = volumeYear[1].text.strip().split()[-1]
            except:
                print("Failed to find issue")
                Issue = ""
            try:
                Year = re.sub(r'[^0-9]+','',volumeYear[0].text.strip())
            except:
        ...
        chinesContent = getPdfWithProxies(f"https://www.opticsjournal.net/J/gxxb/issue/{Year}/{Issue}.html")

        languageList =[currentSoup,chinesContent]

        All_articles=currentSoup.find("div",class_="area contain").findAll("div",class_="contain_left_list")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_title=All_articles[article_index].find("div",class_="contain_left_list_right").find("a").text.strip()
                Article_link="https://www.researching.cn"+All_articles[article_index].find("div",class_="contain_left_list_right").find("a").get("href")
                Article_details=get_soup(Article_link)

                try:
                    DOI=Article_details.find("span",class_="doi").find("a").text.strip()
                except:
                    print("Failed to find doi")
                    DOI = ""
                Page_range=""

                pdf_link=All_articles[article_index].find("div",class_="contain_left_list_rightPDF").find("a").get("href").strip()
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    pdf_content = scraper.get(pdf_link, headers=headers).content
                    output_fimeName = os.path.join(current_out,f"{pdf_count}.pdf")
