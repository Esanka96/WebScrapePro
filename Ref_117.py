...
while url_index < len(url_list):
    try:
        ...
        currentSoup=get_soup(get_soup(url).find("a",string=lambda string: string and "CURRENT ISSUES" in string)["href"])

        try:
            volumeIssue=currentSoup.find("div",class_="page page_issue").h1.text.strip()
            Volume=volumeIssue.split()[1]
            Year=yearCheck(volumeIssue.split("):")[0].split("(")[-1])
            Issue=volumeIssue.split()[3] if volumeIssue.split()[3].isdigit() else ""
            Month=monthCheck(volumeIssue.split("):")[-1].strip().capitalize())
        except:
            Year,Volume,Issue,Month="","","",""
        Day=""

        All_articles=list(filter(lambda x:x.find("a",class_="obj_galley_link pdf"),currentSoup.find("div",class_="sections").findAll("div",class_="obj_article_summary")))
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                try:
                    DOI=Article_details.find("section",class_="item doi").a.text.strip().rsplit("org/",1)[-1]
                except:
                    DOI=""

                error_variable = "pdf link"
                pdf_link=Article_details.find("a",class_="obj_galley_link pdf")["href"]
