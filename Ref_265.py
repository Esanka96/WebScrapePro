...
while url_index < len(url_list):
    try:
        ...
        languageList =["http://alg.xjegi.com/EN/1000-6060/current.shtml",
                       "http://alg.xjegi.com/CN/1000-6060/current.shtml"]

        # currentSoup=get_soup("http://alg.xjegi.com/EN"+get_soup(url).find("article",class_="article").find("div",
        #             class_="detail-content").findAll("table")[1].findAll("tr")[1].find("a")["href"].replace("..",""))

        currentSoup=get_soup("http://alg.xjegi.com/EN/1000-6060/current.shtml")

        try:
            issueVolume = currentSoup.find("div",class_="current-issue").find("span").text.strip().split()
            try:
                Volume = re.sub(r'[^0-9]+','',issueVolume[2])
            except:
            ...
            try:
                ...
                Year = ""
            try:
                Month=monthCheck(issueVolume[-2])
            except:
                print("Failed to find month")
                Month=""

            try:
                Day=dayCheck(re.sub(r'[^0-9]+','',issueVolume[-3]))
            except:
                Day=""
        ...
        All_articles=currentSoup.findAll("li",class_="article-title")
        allPageDetails=currentSoup.findAll("li",class_="article-download")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_title=All_articles[article_index].a.text.strip()
                Article_link=All_articles[article_index].a["href"]
                Article_details=get_soup(Article_link)

                try:
                    Page_range = allPageDetails[article_index].span.text.strip().split(":")[-1].strip()
                except:
                    print("Failed to find page range")
                    Page_range = ""
                try:
                    DOI =Article_details.find("span",class_="doi-doi").a.text.strip()
                except:
                    print("Failed to find doi")
                    DOI = ""

                token = get_token('http://alg.xjegi.com/EN/1000-6060/current.shtml')
                pdf_link="http://alg.xjegi.com/EN/PDF/{}?token={}".format(DOI,token)
