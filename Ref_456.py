...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        baseUrl="https://www.int-res.com"

        currentSoup=get_soup("https://www.int-res.com/"+get_soup(url).find("a",string=lambda text:text and "Latest Volume" in text)["href"])

        Month,Day,Issue="","",""

        try:
            issueVolume = re.sub(r'\s+',' ',currentSoup.find("div", class_="panel-heading ir_bb ir_txt").get_text(strip=True))
            Volume=re.search(r"Vol. (\d+)",issueVolume).group(1)
        except:
            Volume="",""

        All_articles=currentSoup.find("div",class_="journal-index").findAll("a",string=lambda text:text and "MEPS" in text)
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_title=Article_details.find("div",class_="bb").h2.get_text(strip=True)

                try:
                    DOI=Article_details.find("span",string=lambda text:text and "org/" in text).get_text(strip=True).split("org/")[-1]
                except:
                    DOI=""

                try:
                    yearPagerange=Article_details.find("h1",class_="csc-secondHeader").contents[0].get_text(strip=True)
                    Page_range,Year=re.search(r":(\d+-\d+) \((\d{4})\)",yearPagerange).groups()
                except:
                    Year,Page_range="",""

                error_variable = "pdf link"
                pdf_link=baseUrl+Article_details.find("a",class_="article-link")["href"]
