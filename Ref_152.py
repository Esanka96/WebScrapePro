...
while url_index < len(url_list):
    try:
        ...
        currentSoup=get_soup(get_soup(url).find("div",class_="description").find("a").get("href"))

        try:
            volumeYear=currentSoup.find("div",class_="card-body libcomnumber pl-0 pb-0").find("h2").text.strip().split()
        ...
        All_articles=[i for j in currentSoup.findAll("div",class_="section") for i in j.findAll("div",class_="card")]
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_title=All_articles[article_index].find("a",class_="gray").find("h3").text.strip()
                Article_link=All_articles[article_index].find("a",class_="gray").get("href")

                try:
                    Page_range=All_articles[article_index].find("div",class_="libcompage").text.strip()
                except:
                    print("Failed to find page range")
                    Page_range = ""
                try:
                    DOI=re.sub(r'\s+'," ",All_articles[article_index].find("div",class_="logoColorFont").find("a").text.strip()).rsplit("org/",1)[-1]
                except:
                    print("Failed to find doi")
                    DOI = ""

                pdf_link=get_soup(All_articles[article_index].find("a",class_="btnDownload").get("href")).find("a",class_="download").get("href")
