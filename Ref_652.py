...
while url_index < len(url_list):
    try:
        preUrl, url_id = url_list[url_index].split(',')
        url="http://www.mtdzykt.com/en/"
        ...
        attachment = None
        pdf_count = 1
        baseUrl="http://www.mtdzykt.com"

        currentSoup=get_soup(baseUrl+get_soup(url).find("a",string=lambda text : text and "Current Issue" in text)["href"])

        languageList =[currentSoup,
                       get_soup(preUrl)]

        All_articles=currentSoup.find("div",attrs={"id":"issueList"}).findAll("div",class_="article-list")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseUrl+All_articles[article_index].find("div",class_="article-list-title").a["href"]

                error_variable = "article title"
                Article_title=All_articles[article_index].find("div",class_="article-list-title clearfix").a.get_text(strip=True)

                try:
                    volumeIssue=All_articles[article_index].find("div",class_="article-list-time").font.get_text(strip=True).split()
                    try:
                        Year=yearCheck(volumeIssue[0].rstrip(","))
                    except:
                        Year=""
                    try:
                        Page_range=volumeIssue[-1].rstrip(".")
                    except:
                        Page_range=""
                    try:
                        Volume,Issue=re.search(r"(\d+)\((\d+)\):",volumeIssue[1]).groups()
                    except:
                        Volume, Issue = "",""
                except:
                    Year,Page_range,Volume,Issue="","","",""

                try:
                    DOI=All_articles[article_index].find("div",class_="article-list-time").a.get_text(strip=True)
                except:
                    DOI=""

                try:
                    Month=datetime.strptime(Issue,'%m').strftime('%B')
                except:
                    Month=""
                Day=""

                error_variable = "pdf link"
                pdf_link=f"http://www.mtdzykt.com/article/exportPdf?id={get_soup(Article_link).find("meta",attrs={"name":"citation_id"})["content"]}&language=en"
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    pdf_headers = {
                        ...
                        "host": "www.mtdzykt.com",
                        "referer": "http://www.mtdzykt.com/en/",
                        "upgrade-insecure-requests": "1",
