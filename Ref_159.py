...
def resolveCloudFlare():
    firstUrl = "https://www.acpjournals.org/"
    headers1 = {
    ...
    secondUrl = "https://www.acpjournals.org/journal/aim"
    headers2 = {
        "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
        "Accept-Encoding": "gzip, deflate, br, zstd",
        "Accept-Language": "en-GB,en-US;q=0.9,en;q=0.8",
...
while url_index < len(url_list):
    try:
        ...
        headers3 = {
            ...
            "Pragma": "no-cache",
            "Priority": "u=0, i",
            "Referer": "https://www.acpjournals.org/journal/aim",
            "Sec-Ch-Ua": "\"Not/A)Brand\";v=\"8\", \"Chromium\";v=\"126\", \"Google Chrome\";v=\"126\"",
            "Sec-Ch-Ua-Arch": "\"x86\"",
            "Sec-Ch-Ua-Bitness": "\"64\"",
            "Sec-Ch-Ua-Full-Version": "\"126.0.6478.127\"",
        ...
        currentLink = "https://www.acpjournals.org"+BeautifulSoup(session.get(url).content, 'html.parser').find("span",string="ISSUES").findParent()["href"]
        currentSoup=BeautifulSoup(session.get(currentLink,headers=headers3).content, 'html.parser')

        try:
            yearVolume = currentSoup.find("div", class_="current-issue__specifics")
            try:
                yearMonth=yearVolume.find("span").text.strip().split()
                Year=yearMonth[-1]
                Month=yearMonth[0]
                Day=""
            except:
                Year, Month, Day = "", "", ""
            try:
                volumeIssue=yearVolume.findAll("span")[1].text.strip().split(",")
                Volume=volumeIssue[0].split()[-1]
                Issue = volumeIssue[-1].split()[-1]
            except:
                Volume, Issue="", ""
        except:
            if url_check == 0:
        ...
        headers4 = {
            ...
            "Pragma": "no-cache",
            "Priority": "u=0, i",
            "Referer": "https://www.acpjournals.org/toc/aim/current",
            "Sec-Ch-Ua": "\"Not/A)Brand\";v=\"8\", \"Chromium\";v=\"126\", \"Google Chrome\";v=\"126\"",
            "Sec-Ch-Ua-Arch": "\"x86\"",
            "Sec-Ch-Ua-Bitness": "\"64\"",
            "Sec-Ch-Ua-Full-Version": "\"126.0.6478.127\"",
        ...
        TOC = currentSoup.find("div", class_="table-of-content")
        titles = [i for i in TOC.findAll("div", class_="issue-item__body") if i.find("a")]
        letters = TOC.findAll("div", class_="issue-item__letter")
        All_articles = titles + letters
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link="https://www.acpjournals.org"+All_articles[article_index].find("a")["href"]
                Article_details=BeautifulSoup(session.get(Article_link,headers=headers4).content, 'html.parser')

                h1_tag = Article_details.find('h1', {'property': 'name'})
                free_span = h1_tag.find('span', {'class': 'citation__access__type free-access'})
                if free_span:
                    free_span.extract()
                Article_title = h1_tag.text.strip()
                # try:
                #     Page_range = All_articles[article_index].findNext("div",class_="issue-item_page-range").find("li").text.strip().split(":")[-1]
                # except:
                #     Page_range = ""
                # print(Page_range)

                try:
                    DOI = Article_details.find("div", class_="doi").a.text.strip().rsplit("org/",1)[-1]
                except:
                    print("Failed to find doi")
                    DOI = ""
                Page_range=""

                pdf_link = f"https://www.acpjournals.org/doi/pdf/{DOI}?download=true"
