...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        baseUrl="http://kzyjc.alljournals.cn/"
        ...
        try:
            preMonth=currentSoup.find("p",class_="ttime").get_text(strip=True).split()[-1].split("-")[1]
            Month=datetime.strptime(preMonth, '%m').strftime('%B')
        except:
            Month=""
        Day=""

        All_articles=list(map(lambda x:x.find_parent("li"),currentSoup.find("div",class_="list").findAll("div",class_="title")))
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseUrl+All_articles[article_index].find("div",class_="title").a["href"]

                error_variable = "article details"
                Article_details=get_soup(Article_link)

                error_variable = "article title"
                Article_title=Article_details.find("div",attrs={"id":"EnTitleValue"}).get_text(strip=True)

                error_variable = "article id"
                Id=All_articles[article_index].input["value"]

                try:
                    doi_page_range = All_articles[article_index].find('p', class_='ot')
                    try:
                        Page_range_pre = doi_page_range.contents[0]
                        Year,Volume,Issue,Page_range=re.search(r"(\d+),(\d+)\((\d+)\):(\d+-\d+),",Page_range_pre).groups()
                    except:
                ...
                pdf_link=f"http://kzyjc.alljournals.cn/kzyjc/article/pdf/{Id}"

                error_variable = "xml link"
                xml_link=f"http://kzyjc.alljournals.cn/kzyjc/article/export/{Id}?action_type=export&export_type=xml&include_content=2"
