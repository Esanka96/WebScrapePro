...
while url_index < len(url_list):
    try:
        ...
        url="https://www.zfo-online.de/portal/zfo/user/setLocale/en_US?source=%2Fzfo%2Fissue%2Farchive"
        ...
        attachment = None
        pdf_count = 1
        # baseUrl = "https://bjgp.org"

        issue_id=get_soup(url).find("a",class_="title zfo_issue_summary_article")["href"].split("/")[-1]
        currentSoup=get_soup(f"https://www.zfo-online.de/portal/zfo/user/setLocale/en_US?source=%2Fzfo%2Fissue%2Fview%2F{issue_id}")

        try:
            volumeIssue=currentSoup.find("div",class_="page page_issue").h1.text.strip().split()
            Volume=volumeIssue[1]
            Issue=volumeIssue[-2]
            Year=yearCheck(re.sub(r'[^0-9]+', '', volumeIssue[-1]))
        except:
            Year,Volume,Issue="","",""
        Day,Month="",""
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_title=All_articles[article_index].find("h3",class_="title").a.text.strip()

                try:
                    DOI=All_articles[article_index].find("a",{"id":"zfoDoi"}).text.strip().rsplit("org/",1)[-1]
                except:
                    DOI=""

                try:
                    Page_range=All_articles[article_index].find("div",class_="text-muted").i.text.strip()
                except:
                    Page_range=""

                error_variable = "pdf link"
                pdf_link=get_soup(get_soup(Article_link).find("ul",class_="galleys_links").a["href"]).find("a",class_="download")["href"]
