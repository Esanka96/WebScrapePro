...
while url_index < len(url_list):
    try:
        ...
        try:
            volumeYear=currentSoup.find("div",class_="issue-header").find("h4").text.strip().split()
            try:
                ...
                Volume = ""
            try:
                Issue = re.sub(r'[^0-9]+','',volumeYear[-2].split("—")[0])
            except:
            ...
            try:
                ...
                Year = ""
            try:
                Month=monthCheck(volumeYear[-2].split("—")[-1])
            except:
        ...
        All_articles=currentSoup.find("div",{"id":"currentContainer"}).findAll("a",class_="mr-1 ft-14")
        allPdfLinks=currentSoup.find("div",{"id":"currentContainer"}).findAll("span",class_="pdf-link")[1:]
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link="https://wwwnc.cdc.gov"+All_articles[article_index].get("href")
                Page_range = ""
                try:
                    DOI=get_soup(Article_link).find("p",{"id":"article-doi-footer"}).text.strip().rsplit("DOI: ",1)[-1]
                except:
                    print("Failed to find doi")
                    DOI = ""

                pdf_link="https://wwwnc.cdc.gov"+allPdfLinks[article_index].find("a")["href"]
