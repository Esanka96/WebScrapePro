...
while url_index < len(url_list):
    try:
        ...
        currentSoup=get_soup("https://insight.jci.org"+get_soup(url).find("a",{"id":"topmenu_current_issue"})["href"])

        try:
            volumeYear=currentSoup.find("ul",class_="no-bullet").find("li").text.strip().split()
            try:
            ...
            try:
                ...
                Issue = ""
            try:
                yearMonth=currentSoup.find("ul",class_="no-bullet").find_previous("h4").get_text(strip=True)
                Month,Day,Year=re.search(r"Issue published (\w+) (\d+), (\d{4})",yearMonth).groups()
            except:
                Month,Day,Year="","",""
        ...
        allMetaData=currentSoup.findAll("div",class_="article-metadata")
        All_articles=currentSoup.findAll("h5",class_="article-title")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link="https://insight.jci.org"+All_articles[article_index].a.get("href")

                try:
                    Identifier=re.sub(r'\s+'," ",allMetaData[article_index].find("a",{"id":"article_metadata"}).next_sibling[1:-2])
                except:
                    print("Failed to find identifier")
                    Identifier = ""
                try:
                    DOI=allMetaData[article_index].find("a",{"id":"article_metadata"}).findNext("a").text.strip().rsplit("org/",1)[-1]
                except:
                    print("Failed to find doi")
                    DOI = ""
                Page_range=""

                pdf_link="https://insight.jci.org"+get_soup("https://insight.jci.org"+allMetaData[article_index].find("div",class_="article-links").find("a",string="PDF")["href"]).find("a",string="Download")["href"]
