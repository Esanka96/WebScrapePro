...
import Login_with_credentials
from tenacity import retry, stop_after_attempt, wait_fixed

new_session = requests.session()
def getSession():
    url = 'http://www.annclinlabsci.org/'
    ...
    mainHeaders = {
        ...
        #"Cookie": "_ga=GA1.1.928527854.1719893686; cks=allowed; JSESSIONID=552DF6505A71DE934D0A876B8FD118DA.wavmfe600.highwire.org; _ga_1EQTKV57EP=GS1.1.1719893686.1.1.1719895439.0.0.0",
        "Host": "www.annclinlabsci.org",
        "Pragma": "no-cache",
    ...
    currentUrl="http://www.annclinlabsci.org/content/current"
    ...
    currentHeaders = {
        ...
        "Cache-Control": "no-cache",
        "Connection": "keep-alive",
        #"Cookie": "_ga=GA1.1.928527854.1719893686; cks=allowed; JSESSIONID=552DF6505A71DE934D0A876B8FD118DA.wavmfe600.highwire.org; _ga_1EQTKV57EP=GS1.1.1719893686.1.1.1719895599.0.0.0",
        "Host": "www.annclinlabsci.org",
        "Pragma": "no-cache",
        "Referer": "http://www.annclinlabsci.org/",
        "Upgrade-Insecure-Requests": "1",
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36"
    }

    new_session.get(currentUrl,headers=currentHeaders)

    post_url='http://www.annclinlabsci.org/login'
    ...
    loginHeaders = {
        ...
        #"Cookie": "_ga=GA1.1.928527854.1719893686; cks=allowed; JSESSIONID=552DF6505A71DE934D0A876B8FD118DA.wavmfe600.highwire.org; _ga_1EQTKV57EP=GS1.1.1719893686.1.1.1719895738.0.0.0",
        "Host": "www.annclinlabsci.org",
        "Origin": "http://www.annclinlabsci.org",
        "Pragma": "no-cache",
        "Referer": "http://www.annclinlabsci.org/content/current",
        "Upgrade-Insecure-Requests": "1",
        "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/126.0.0.0 Safari/537.36"
    }
    ...
    data = {
        "username": "elsevier",
        "code": "source",
        #"uri": "http://www.annclinlabsci.org/content/current"
        # "x": 19,
        # "y": 7
    }

    new_session.post(post_url,data=data,headers=loginHeaders)
    new_session.get(currentUrl)
...
while url_index < len(url_list):
    try:
        ...
        currentSoup=get_soup("http://www.annclinlabsci.org"+get_soup(url).find("a",{"title":"CURRENT ISSUE"})["href"])
        ...
        try:
            volumeYear = currentSoup.find("span",class_="toc-top-pub-date").text.strip().split()
            try:
                Volume = re.sub(r'[^0-9]+','',currentSoup.find("span",class_="toc-citation-volume").text.strip())
            except:
                print("Failed to find volume")
                Volume = ""
            try:
                Issue = re.sub(r'[^0-9]+','',currentSoup.find("span",class_="toc-citation-issue").text.strip())
            except:
                print("Failed to find issue")
                Issue = ""

            try:
                Year = yearCheck(re.sub(r'[^0-9]+','',volumeYear[-1]))
            except:
                print("Failed to find year")
                Year = ""
            try:
                Month=monthCheck(volumeYear[0].split("-")[0])
            except:
        ...
        All_articles=[ele for i in currentSoup.findAll("ul",class_="cit-list") for ele in i.findAll("li",class_="cit from-current-issue toc-cit")]
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_title=re.sub(r'\s+'," ",All_articles[article_index].find("h4",class_="cit-title-group").text.strip())

                try:
                    Page_range = All_articles[article_index].find("span",class_="cit-pages").text.strip()
                except:
                    print("Failed to find page range")
                    Page_range = ""
                DOI=""

                pdf_link="http://www.annclinlabsci.org/"+All_articles[article_index].find("a",string="Full Text (PDF)")["href"].replace("+html","#page=1&view=FitH")
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    pdf_content = new_session.get(pdf_link).content
