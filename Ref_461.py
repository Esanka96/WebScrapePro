...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        baseUrl="http://hit.alljournals.cn"

        currentSoup=get_soup(url)
        languageList =[currentSoup]

        All_articles=currentSoup.find("h5",class_="Lanm").find_parent("ul").findAll("li")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                try:
                    preMonth=Art_index.find("p").contents
                    try:
                        DOI=re.search(r"DOI:(.+)",preMonth[0]).group(1)
                    except:
                        DOI=""
                    try:
                        Year,Volume,Issue,Page_range=re.search(r":(\d{4}),(\d+)\((\d+)\):(\d+-\d+)",preMonth[2]).groups()
                    except:
                        Year,Volume,Issue,Page_range="","","",""
                except:
                    DOI,Page_range,Year,Volume,Issue="","","","",""
                Month,Day="",""

                error_variable = "pdf link"
                pdf_link=baseUrl+Art_index.find("a",string=lambda text:text and "PDF" in text)["href"]
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    pdf_headers = {
                        ...
                        #"cookie": "ASP.NET_SessionId=krtxb231mlzweqlcq0y1zrtr",
                        "host": "hit.alljournals.cn",
                        "pragma": "no-cache",
