...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        baseUrl="https://ufn.ru"

        lastIssue=get_soup(url)
        currentIssue=lastIssue.find("td",title=lambda text : text and "Volume" in text).find_parent("tr").findAll("td",title=lambda text : text and "pages" in text)[-1]

        currentSoup=get_soup(baseUrl+currentIssue.a["href"])
        try:
            monthYear=currentSoup.findAll("h1",class_="inline")
            Year=yearCheck(monthYear[2].get_text(strip=True))
            Month=monthCheck(monthYear[-1].get_text(strip=True))
        except:
            Month,Year="",""
        Day=""

        try:
            Issue=currentIssue.a.get_text(strip=True)
        except:
            Issue=""

        volumeLink=lastIssue.find("td",title=lambda text : text and "Volume" in text).find_parent("tr").find("td")

        try:
            Volume=volumeLink.get_text(strip=True).split()[-1]
        except:
            Volume=""

        eng_link=baseUrl+volumeLink.a["href"]
        rus_link=eng_link.replace("en","ru")

        languageList=[get_soup(eng_link),get_soup(rus_link)]

        All_articles=[link for i in currentSoup.findAll("p",class_="articles") for link in i.findAll("a")]
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseUrl+Art_index["href"]

                error_variable = "article details"
                Article_details=get_soup(Article_link)

                error_variable = "article title"
                Article_title=Article_details.find("h1",attrs={"itemprop":"headline"}).get_text(strip=True)

                try:
                    DOI=Article_details.find("a",href=lambda link : link and "doi.org" in link).get_text(strip=True)
                except:
                    DOI=""

                try:
                    Page_range=Article_details.find("table",class_="cit_table").find_previous_sibling("b").find_next_sibling(string=True).get_text(strip=True).split()[0]
                except:
                    Page_range=""

                error_variable = "pdf link"
                pdf_link=baseUrl+Article_details.find("a",class_="file-link track-fulltext-download pdf")["href"]
