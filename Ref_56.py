...
while url_index < len(url_list):
    try:
        ...
        Volume_issue=current_soup.find('div',class_='description clearfix').text.strip().split(',')
        Volume=re.sub(r'[^0-9]+','',Volume_issue[0])
        Issue=re.sub(r'[^0-9]+','',Volume_issue[1])
        Year_Month=Volume_issue[2].split('\n')[0].split()
        Year=Year_Month[-1]
        Month = month_check(Year_Month[0].split('-')[0].lower(),Year_Month[0].split('-')[0])

        All_articles_pre=current_soup.find('section',class_='bloc b-sommaire')

        if All_articles_pre.find("div",{"id":"q_quiz"}):
            last_ele=All_articles_pre.find("div",{"id":"q_quiz"})
            last_ele.extract()

        All_articles = All_articles_pre.findAll('p')
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link='https://www.jle.com'+All_articles[article_index].find('a').get('href')
                Article_title = All_articles[article_index].find('a').text.strip()
                Article_details=get_soup(Article_link)

                try:
                    DOI=Article_details.find('ul',class_='resume').findAll('li')[-3].text.strip().rsplit('DOI : ',1)[-1]
                except:
                    DOI=""

                try:
                    Page_range=check_page_range(Article_details.find('ul',class_='resume').findAll('li')[-2].text.strip().rsplit('Page(s) : ',1)[-1])
                except:
                    Page_range=""

                pdf_link='https://www.jle.com'+Article_details.find('ul',class_='tools').find('li').find('a').get('href')
