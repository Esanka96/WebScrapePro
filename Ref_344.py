...
while url_index < len(url_list):
    try:
        ...
        url="http://lygcxb.njfu.edu.cn/api/api/Web/LastIssueList?_t=&PageSize=0&IssueId=0"
        ...
        All_articles=currentSoup=json.loads(get_soup(url).text)["Data"]
        ...
        first_line=f"{All_articles[0]["PublishYear"]} Vol.{All_articles[0]["Volume"]} Issue:{All_articles[0]["IssueNumE"]}"
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = f"http://lygcxb.njfu.edu.cn/en/#/digest?ArticleID={All_articles[article_index]["ArticleID"]}"

                error_variable = "article title"
                Article_title=All_articles[article_index]["TitleE"]

                error_variable = "article volume and issue"
                Volume=All_articles[article_index]["Volume"]
                Issue=All_articles[article_index]["IssueNumE"]
                ...
                html_body_en += f"<a href=href='{Article_link}' style='text-decoration:none;' color:#00f;'><h3 >{Article_title} [{Page_range}]</h3></a>\n"
                html_body_en += f"<p>{All_articles[article_index]['AuthorsListE']}</p>\n"
                html_body_en += f"<p>DOI: <a href='https://doi.org/{DOI}'>{DOI}</a></p>\n"
                html_body_en += f"<p>Abstract  Viewed({All_articles[article_index]["View_Times"]})  [HTML]({All_articles[article_index]["HtmlRead_Times"]}) Export  PDF ({All_articles[article_index]["Read_Times"]})  Scan</p>\n"
                html_body_en += "</div>\n"

                html_body_cn += f"<div class='article''>\n"
                html_body_cn += f"<a href=href='{Article_link}' style='text-decoration:none;' color:#00f;><h3 >{All_articles[article_index]["Title"]} [{Page_range}]</h3></a>\n"
                ...
                pdf_link=f"http://lygcxb.njfu.edu.cn/api/api/Web/OpenArticleFilebyGuidNew?Id={All_articles[article_index]["PDFFileNameUrlGUID"]}"
