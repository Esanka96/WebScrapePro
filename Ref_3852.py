...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        baseUrl="http://spyswjs.cnjournals.com/"

        currentSoup=get_soup(url)

        languageList =[currentSoup,
                       get_soup("http://spyswjs.cnjournals.com/spyswjs/home")]

        try:
            volumeIssue=currentSoup.find("span",class_="banTitle").get_text(strip=True)
            Year,Volume,Issue=re.search(r"(\d{4}) Volume (\d+), Issue (\d+)",volumeIssue).groups()
            Month, Day = "", ""
        except:
            Year,Volume,Issue,Month,Day="","","","",""

        All_articles=currentSoup.find("div",class_="bd check_ww").ul.findAll("div",class_="slideTxtBox_list")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseUrl+Art_index.find("div",class_="slideTxtBox_list_title").a["href"]

                error_variable = "article title"
                Article_title=Art_index.find("div",class_="slideTxtBox_list_title").a.get_text(strip=True)

                try:
                    Page_range = Art_index.find("div",class_="slideTxtBox_list_time").i.get_text(strip=True).split(":")[-1]
                except:
                    Page_range=""

                try:
                    DOI=get_soup(Article_link).find("a",attrs={"id":"DOIValue"}).get_text(strip=True)
                except:
                    DOI=""
