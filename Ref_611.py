...
while url_index < len(url_list):
    try:
        ...
        currentSoup=get_soup(get_soup(url).find("a",string=lambda text:text and "Current" in text)["href"])

        try:
            preMonth=re.sub(r"\s+"," ",currentSoup.find("div",class_="page page_issue").h1.get_text(strip=True))
            Volume,Issue,Year,Month=re.search(r"Vol. (\d+) No. (\d+) \((\d{4})\): IJMS \[(\w+)",preMonth).groups()
        except:
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = Art_index.find("h3",class_="title").a["href"]

                error_variable = "article title"
                Article_title=Art_index.find("h3",class_="title").a.get_text(strip=True)

                error_variable = "article details"
                Article_details=get_soup(Article_link)

                try:
                    Page_range=Art_index.find("div",class_="pages").get_text(strip=True)
                except:
                    Page_range=""

                try:
                    DOI=Article_details.find("section",class_="item doi").a.get_text(strip=True).split("org/")[-1]
                except:
                    DOI=""

                error_variable = "pdf link"
                pdf_link=get_soup(Article_details.find("a",class_="obj_galley_link pdf")["href"]).find("a",class_="download")["href"]
