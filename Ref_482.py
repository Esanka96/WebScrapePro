...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        baseurl="https://journals.ashs.org"

        currentSoup=get_soup(url)

        try:
            volumeIssue=currentSoup.find("span",string=lambda x:x and "Volume" in x).text.strip()
            pattern = r"Volume (\d+) \((\d{4})\): Issue ([\w\d]+) \((\w+)"
            Volume,Year,Issue,Month=re.search(pattern,volumeIssue).groups()
            try:
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseurl+All_articles[article_index].find("h1",class_="text-display4").a["href"]

                error_variable = "article title"
                Article_title=All_articles[article_index].find("h1",class_="text-display4").a.text.strip()

                error_variable = "article details"
                Article_details=get_soup(Article_link)

                try:
                    Page_range = All_articles[article_index].find('dl', class_='pagerange c-List__items').findAll("span")[-1].get_text(strip=True)
                except:
                    Page_range=""

                try:
                    DOI=All_articles[article_index].find('dl', class_='doi c-List__items').a.text.strip().rsplit("org/",1)[-1]
                except:
                    DOI=""

                error_variable = "pdf link"
                pdf_link=baseurl+Article_details.find("a",title="Download PDF")["href"]
