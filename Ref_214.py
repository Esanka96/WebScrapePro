...
def getVolume(list):
    ...
    for i,item in enumerate(list):
        if "Vol." in item:
            Volume=list[i+1].rstrip(",")
        if any(keyword in item for keyword in ["Núm.", "NÚM.", "No."]):
            Issue=list[i+1].rstrip(",")
        if year_pattern.search(item):
            Year = year_pattern.search(item).group(0).strip('()')
...
while url_index < len(url_list):
    try:
        ...
        currentLink=next(
            (
                ele for ele in get_soup(url).find("ul",{"class":"issues_archive"}).findAll("h2")
                if any(x in ele.get_text(strip=True) for x in ["NÚM","Núm","VOL","Vol"])
            ),None
        ).find("a")

        lastLink=currentLink["href"]
        currentId=lastLink.split("/")[-1]
        urlValue=url.split("/")[-3]
        Eng_link = f"http://ojs.uv.es/index.php/{urlValue}/user/setLocale/en_US?source=%2Findex.php%2F{urlValue}%2Fissue%2Fview%2F{currentId}"

        currentSoup=get_soup(Eng_link)
        if currentSoup.find("div",{"id":"issueCoverImage"}):
            currentSoup = get_soup(Eng_link+"/showToc")

        try:
            issueVolume = currentSoup.find("div",class_="page page_issue").h1.get_text(strip=True).split(":")[0].split()
            Year,Volume,Issue=getVolume(issueVolume)
        except:
            Year,Volume,Issue="","",""
        Month, Day="",""

        All_articles=currentSoup.findAll("div",class_="obj_article_summary")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_title=All_articles[article_index].find("h3",class_="title").get_text(strip=True)
                Article_link = All_articles[article_index].find("h3",class_="title").a["href"]
                ...
                pdf_link=get_soup(All_articles[article_index].findAll("a",class_="obj_galley_link pdf")[-1]["href"].replace("https","http")).find("a",class_="download")["href"].replace("https","http")

                try:
                    doiLink=Article_link.replace("https","http")
                    DOI=get_soup(doiLink).find("section",class_="item doi").find("a").get_text(strip=True).rsplit("org/",1)[-1]
                except:
                    DOI=""
