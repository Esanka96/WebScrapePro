...
while url_index < len(url_list):
    try:
        ...
        try:
            preMonth=re.sub(r"\s+"," ",currentSoup.find("h1",class_="den-dark-blue").get_text(strip=True))
            Issue,Volume,Year=re.search(r"Issue (\d+) Volume (\d+) \((\d{4})\)",preMonth).groups()
        except:
            Year,Volume,Issue="","",""
        Month,Day="",""

        All_articles=currentSoup.findAll("div",class_="row no-gutters")[-1].findAll("div",class_="col-12 col-md-6")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = Art_index.find("div",class_="flex-grow-1").a["href"]

                error_variable = "article details"
                Article_details=get_soup(Article_link)

                error_variable = "article title"
                Article_title=Article_details.find("h1").get_text(strip=True)

                try:
                    Page_range="{}-{}".format(Article_details.find("meta",attrs={"name":"citation_firstpage"})["content"],
                                              Article_details.find("meta",attrs={"name":"citation_lastpage"})["content"])
                except:
                    Page_range=""

                try:
                    DOI=Article_details.find("meta",attrs={"name":"citation_doi"})["content"]
                except:
                    DOI=""

                error_variable = "pdf link"
                pdf_link=Article_details.find("meta",attrs={"name":"citation_pdf_url"})["content"]
