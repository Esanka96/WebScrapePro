...
def resolveCloudFlare():
    firstUrl = "https://journals.jps.jp/loi/jpsj"
    headers1 = {
        "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
        "Accept-Encoding": "gzip, deflate, br, zstd",
        "Accept-Language": "en-GB,en-US;q=0.9,en;q=0.8",
        "Cache-Control": "no-cache",
        # "Cookie": "MAID=djtTRC5WbkFmTsDkochSCw==; cookieconsent_status=dismiss; MAID_DISC=djtTRC5WbkFmTsDkochSCw; MACHINE_LAST_SEEN=2024-07-09T00%3A44%3A04.179-07%3A00; JSESSIONID=9A2341C20A834A5D2A5F4F9E4361F0D3; __cf_bm=fau8UgQwYlkHi4N49S9M8k075ZIMjaItrtKrMzQjStk-1720511044-1.0.1.1-m.Um8WwKDSBmerXLM0ZCgHzIG3pviym609c_6vW5xMAHgnd67pv77zLIDHMdkDSTMCr.uqRurkRV2FU2sGxarQ; cf_clearance=LCG7643dLppZlEDiaBDVTeCybzK3NleBJRYkwcgkaqM-1720511047-1.0.1.1-ayr6tWQ4CmZN2v_9j2HlDKoPCy8vg5c11yHrHWTK_fWlPyKJF6ZofjZ1TKqJP38nk4fCbOX5n4nF80qG2D.bug",
        "Pragma": "no-cache",
        "Priority": "u=0, i",
    ...
    secondUrl = "https://journals.jps.jp/toc/jpsj/current"
    headers2 = {
        'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
        'Accept-Encoding': 'gzip, deflate, br, zstd',
        'Accept-Language': 'en-GB,en-US;q=0.9,en;q=0.8',
        ...
        'Pragma': 'no-cache',
        'Priority': 'u=0, i',
        'Referer': 'https://journals.jps.jp/loi/jpsj',
        'Sec-Ch-Ua': '"Not/A)Brand";v="8", "Chromium";v="126", "Google Chrome";v="126"',
        'Sec-Ch-Ua-Mobile': '?0',
        'Sec-Ch-Ua-Platform': '"Windows"',
        'Sec-Fetch-Dest': 'document',
...
while url_index < len(url_list):
    try:
        ...
        resolveCloudFlare()
        response=session.get("https://journals.jps.jp/toc/jpsj/current")
        currentSoup=BeautifulSoup(response.content, 'html.parser')

        try:
            issueVolume = re.sub(r'\s+',' ',currentSoup.find("p", class_="issueInfo").text.strip()).split()
            try:
                Volume = re.sub(r'[^0-9]+','',issueVolume[1])
            except:
                print("Failed to find volume")
                Volume = ""
            try:
                Issue = re.sub(r'[^0-9]+','',issueVolume[3])
            except:
                print("Failed to find issue")
                Issue = ""

            try:
                Year = yearCheck(re.sub(r'[^0-9]+','',issueVolume[-1]))
            except:
                print("Failed to find year")
                Year = ""
            try:
                Month=monthCheck(re.sub(r'[^a-zA-Z]+','',issueVolume[-3]))
            except:
                print("Failed to find month")
                Month=""

            try:
                Day = dayCheck(re.sub(r'[^0-9]+', '', issueVolume[-2]))
            except:
                Day = ""
        ...
        All_articles=currentSoup.find("div",class_="articleGroup").findAll("div",class_="item clearfix")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_title=All_articles[article_index].find("span",class_="hlFld-Title").text.strip()

                try:
                    doiId = All_articles[article_index].find("p", class_="float-right").findNext("p").text.strip().split(",")
                    try:
                        Identifier = doiId[0].strip()
                    except:
                    ...
                    print("Failed to find doi and identifier")
                    DOI,Identifier = "",""
                Page_range = ""

                pdf_link="https://journals.jps.jp/doi/pdf/{}?download=true".format(DOI)
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    responsePDF = session.get(pdf_link)
                    output_fimeName = os.path.join(current_out, f"{pdf_count}.pdf")
                    with open(output_fimeName, "wb") as pdf_file:
                        pdf_file.write(responsePDF.content)
                    # output_fimeName = os.path.join(current_out, f"{pdf_count}.pdf")
