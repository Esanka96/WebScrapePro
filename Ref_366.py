...
while url_index < len(url_list):
    try:
        ...
        currentSoup=get_soup(get_soup(url).find("a",string=lambda link:link and "Current" in link)["href"])

        languageList =[currentSoup,get_soup(currentSoup.find("div",class_="content").findAll("a")[-1]["href"])]

        try:
            volumeIssue=currentSoup.find("div",class_="page page_issue").h1.get_text(strip=True)
            pattern=r"Vol. (\d+) No. (\d+) \((\d{4})\)"
            Volume,Issue,Year=re.search(pattern,volumeIssue).groups()
        except:
            Year,Issue,Volume="","",""
        Day,Month="",""
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_title=All_articles[article_index].find("h3",class_="title").a.get_text(strip=True)
                ...
                try:
                    DOI=All_articles[article_index].find('div', class_='item doi').a.get_text(strip=True).rsplit("org/",1)[-1]
                except:
                    DOI=""

                error_variable = "pdf link"
                pdf_link=get_soup(All_articles[article_index].find('a', class_='obj_galley_link pdf')["href"]).find("a",class_="download")["href"]
