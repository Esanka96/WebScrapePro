...
def resolveCloudFlare():
    firstUrl = "https://jcsm.aasm.org/journal/jcsm"
    headers1 = {
    ...
    secondUrl = "https://jcsm.aasm.org/toc/jcsm/current"
    headers2 = {
        "Accept": "text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7",
        "Accept-Encoding": "gzip, deflate, br, zstd",
        "Accept-Language": "en-GB,en-US;q=0.9,en;q=0.8",
...
while url_index < len(url_list):
    try:
        ...
        try:
            preMonth=currentSoup.find("h5",class_="teaser__group-title").get_text(strip=True)
        ...
        All_articles = [article for first in currentSoup.findAll("section",class_="table-of-content__section") for article in first.findAll("div",class_="issue-item")]
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_link=baseUrl+Art_index.find("h5",class_="issue-item__title").a["href"]
                Article_title=re.sub(r"\s+"," ",Art_index.find("h5",class_="issue-item__title").a.get_text(strip=True))

                try:
                    Page_range = Art_index.find("div",class_="issue-item__pages").get_text(strip=True).split("pp")[-1].strip()
                except:
                    Page_range = ""
                ...
                pdf_link = f"https://jcsm.aasm.org/doi/pdf/{DOI}"
