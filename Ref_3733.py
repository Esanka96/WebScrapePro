...
while url_index < len(url_list):
    try:
        ...
        languageList =[currentSoup,
                       get_soup("https://www.xdqb.net/CN/home")]

        try:
            preVolumeIssue=currentSoup.find("div",class_="n-j-q").contents
            try:
                Year,Volume,Issue=re.search(r"(\d{4}), (\d+) \((\d+)\)",preVolumeIssue[1].get_text(strip=True)).groups()
            except:
                Year,Volume,Issue="","",""
            try:
                Day,Month=re.search(r"Publication date:(\d+) (\w+)",preVolumeIssue[3].get_text(strip=True)).groups()
            except:
                Day,Month="",""
        except:
            Year,Volume,Issue,Month,Day="","","","",""

        All_articles=currentSoup.find("ul",class_="article-list").findAll("li",class_="noselectrow")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = All_articles[article_index].find("div",class_="j-title-1").a["href"]

                error_variable = "article title"
                Article_title=All_articles[article_index].find("div",class_="j-title-1").a.text.strip()

                try:
                    doi_page_range = All_articles[article_index].find('div', class_='j-volumn-doi')
                    try:
                        Page_range = doi_page_range.find("span",class_="j-volumn").get_text(strip=True).split()[-1].rstrip(".")
                    except:
                        Page_range=""

                    try:
                        DOI=doi_page_range.a.text.strip().rsplit("org/",1)[-1]
                    except:
                        DOI=""
                except:
                    DOI,Page_range="",""

                pdfDoi=All_articles[article_index]["id"].split("art")[-1] if DOI=="" else DOI

                error_variable = "pdf link"
                token=get_token("https://www.xdqb.net/")
                pdf_link="https://www.xdqb.net/EN/PDF/{0}?token={1}".format(pdfDoi,token)
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    pdf_headers = {
                        ...
                        #"cookie": "Secure; JSESSIONID=4F94BC53EFD0222F724D39745284931D; wkxt3_csrf_token=888e69ec-2709-4f31-b21e-9e5f7f5e043a; Secure",
                        "host": "www.xdqb.net",
