...
def getVolume(pdf_link,Article_link):
    try:
        ...
        with pdfplumber.open(pdf_content) as pdf:
            ...
            text = page.split("\n")
            try:
                Month = monthCheck(text[3].split("，")[0])
            except:
                Month = ""
            ...
            try:
                ...
                for i in text:
                    if "doi" in i:
                        date_str = re.search(r'file_no=(\d{8})', Article_link).group(1)
                        DOI = f"10.12134/j.dzykt.{date_str[:4]}.{date_str[4:6]}.0{date_str[6:8]}"
                        break
            except:
                DOI = ""
    except:
...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        baseUrl="http://www.dzykt.com/dzykten/ch/"

        currentSoup=get_soup(url)

        languageList =[currentSoup,
                       get_soup("http://www.dzykt.com/dzyktcn/ch/index.aspx")]


        All_articles=currentSoup.find("div",attrs={"id":"central-zhong"}).findAll("li",attrs={"id":"ch3"})
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseUrl+All_articles[article_index].find("a")["href"]
                Article_details=get_soup(Article_link)

                error_variable = "article title"
                Article_title=Article_details.find("span",attrs={"id":"EnTitle"}).get_text(strip=True)

                try:
                    volume_age_range = Article_details.find("span",attrs={"id":"ReferenceText"}).get_text(strip=True).split(",")
                    try:
                        Year=yearCheck(volume_age_range[-2])
                    except:
                        Year=""
                    try:
                        Page_range=volume_age_range[-1].split(":")[-1]
                    except:
                        Page_range=""
                    try:
                        Volume, Issue = re.search(r"(\d+)\((\d+)\)",volume_age_range[-1].split(":")[0]).groups()
                    except:
                        Volume,Issue="",""
                except:
                    Volume,Issue,Page_range,Year="","","",""

                error_variable = "pdf link"
                pdf_link="http://www.dzykt.com/dzykten/ch/reader/"+Article_details.find("span",attrs={"id":"URL"}).a["href"]

                DOI,Month=getVolume(pdf_link,Article_link)
