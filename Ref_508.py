...
while url_index < len(url_list):
    try:
        ...
        url="http://xb.chinasmp.com/EN/1001-1595/current.shtml"
        ...
        languageList =[currentSoup,
                       get_soup("http://xb.chinasmp.com/CN/1001-1595/current.shtml")]

        try:
            volumeIssue=currentSoup.find("div",class_="njq").text.strip()
            pattern=r"(\d+) (\w+) (\d{4}), Volume (\d+) Issue (\d+)"
            Day,Month,Year,Volume,Issue=re.search(pattern,volumeIssue).groups()
        except:
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = All_articles[article_index].find("a",class_="biaoti_en")["href"]

                error_variable = "article title"
                Article_title=All_articles[article_index].find("a",class_="biaoti_en").text.strip()

                try:
                    doi_page_range = All_articles[article_index].find('dd', class_='kmnjq')
                    try:
                        Page_range = re.search(r"\): (.*?). doi",re.sub(r'\s+'," ",doi_page_range.text.strip())).group(1)
                    except:
                        Page_range=""

                    try:
                        DOI=doi_page_range.a.text.strip()
                    except:
                        DOI=""
                except:
                    DOI,Page_range="",""
                ...
                token=All_articles[article_index]["id"].split("art")[-1]
                pdf_link=f"http://xb.chinasmp.com/EN/article/downloadArticleFile.do?attachType=PDF&id={token}"
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    pdf_headers = {
                        ...
                        #"cookie": "Secure; acw_tc=1a0c65e217295751780634106e003a7f3f78cbdb892c1e236b8abf7c4d2dba; JSESSIONID=6F0564B0EC3DBA549DF4A4B60E718D58; wkxt3_csrf_token=4bb13f21-3dc6-4b10-81dd-f286b71f1c11; Secure",
                        "host": "xb.chinasmp.com",
                        "pragma": "no-cache",
