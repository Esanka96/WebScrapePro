...
while url_index < len(url_list):
    try:
        ...
        Year,preVolume,preIssue=current_soup.find('h2',class_='journalIssue text-center commontit').find('p').text.strip().split(',')
        Volume=re.sub(r'[^0-9]+','',preVolume)
        Issue = re.sub(r'[^0-9]+', '', preIssue)

        All_articles=current_soup.find('div',class_='articleListBox active base-catalog').findAll('div',class_='article-list')
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                preYear, prePageRange, DOI = re.sub(r'\s+', '', All_articles[article_index].find('div',class_='article-list-time').text.strip()).split(':')
                Page_range = prePageRange.rstrip('.doi')
                pdf_link = 'http://www.cjsh.org.cn/article/exportPdf?id=' + All_articles[article_index].get('id') + '&language=en'
