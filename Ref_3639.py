...
while url_index < len(url_list):
    try:
        ...
        languageList =[currentSoup,
                       get_soup("https://www.resci.cn/CN/current")]

        try:
            preVolumeIssue=currentSoup.find("span",class_="n-j-q").contents
            try:
                Year,Volume,Issue=re.search(r"(\d{4}) Volume (\d+) Issue (\d+)",preVolumeIssue[0].get_text(strip=True)).groups()
            except:
                Year,Volume,Issue="","",""
            try:
                Month=preVolumeIssue[2].get_text(strip=True).split()[-2]
            except:
                Month=""
        except:
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                token=get_token("https://www.resci.cn/EN/current")
                pdf_link="https://www.resci.cn/EN/PDF/{0}?token={1}".format(DOI,token)
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    pdf_headers = {
                        ...
                        #"cookie": "Secure; JSESSIONID=6C9E94CCA8305BEEA178C4BE04AC623F; wkxt3_csrf_token=a9003d3f-43cd-41e4-a7b9-2dbd816f4084; Secure",
                        "host": "www.resci.cn",
