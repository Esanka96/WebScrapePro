...
while url_index < len(url_list):
    try:
        ...
        try:
            volumeIssue=currentSoup.find("strong",string=lambda x:x and "No." in x).text.strip().split()
            Month=monthCheck(volumeIssue[0])
            Day=dayCheck(volumeIssue[1].rstrip(","))
            Year=yearCheck(volumeIssue[2])
            Issue=volumeIssue[-1]
        except:
            Year,Day,Issue,Month="","","",""

        try:
            Volume=re.search(r"\((\d+)\)",currentSoup.find("h1",class_="page-title").get_text(strip=True).split()[-1]).group(1)
        except:
            Volume=""

        All_articles=currentSoup.find("ul",class_="false").findAll("li")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseurl+All_articles[article_index].a["href"]
                ...
                try:
                    Page_range = Article_details.find('div', class_='dateline').p.get_text(strip=True).split(";")[-1]
                except:
                    Page_range=""
                ...
                pdf_link=baseurl+Article_details.find("a",href=lambda link:link and ".pdf" in link)["href"]
