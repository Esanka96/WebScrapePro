...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        baseUrl="https://filclass.ru"

        yearUrl=baseUrl+get_soup(url).find("h3",class_="page-header item-title").a["href"]
        ...
        currentSoup=get_soup(baseUrl+get_soup(yearUrl).find("h3",class_="page-header item-title").a["href"])

        try:
            preMonth=currentSoup.find("span",class_="subheading-category").get_text(strip=True)
            Volume,Issue=re.search(r"Vol. (\d+) ⋅ №(\d+)",preMonth).groups()
        except:
            Volume,Issue="",""
        Month,Day="",""

        All_articles = currentSoup.find("div",class_="items-leading clearfix").findAll("div",class_="articleblock")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseUrl+Art_index.find("h2",attrs={"itemprop":"name"}).a["href"]

                error_variable = "article title"
                Article_title=Art_index.find("h2",attrs={"itemprop":"name"}).a.get_text(strip=True)

                error_variable = "article details"
                Article_details=get_soup(Article_link)

                try:
                    DOI=Article_details.find("div",class_="doi").get_text(strip=True).split("DOI:")[-1]
                except:
                    DOI=""

                try:
                    Page_range=Art_index.find("div",class_="numstr").get_text(strip=True)
                except:
                    Page_range=""

                error_variable = "pdf link"
                pdf_link=baseUrl+Article_details.find("a",string=lambda href:href and "PDF" in href)["href"]
