...
while url_index < len(url_list):
    try:
        ...
        url="https://www.zjujournals.com/agr/EN/1008-9209/current.shtml"
        ...
        languageList =[currentSoup,
                       get_soup("https://www.zjujournals.com/agr/CN/1008-9209/current.shtml")]

        try:
            try:
                monthYear=currentSoup.find("span",class_="date").script.get_text(strip=True)
                Year,Month,dayValue=re.search(r"\('(\d{4})-(\d+)-(\d+) ",monthYear).groups()
                Month = datetime.strptime(Month, '%m').strftime('%B')
            except:
                Year,Month,Day="","",""
            Day=""
            try:
                issueVolume=currentSoup.find("span",class_="date").contents[2].get_text(strip=True)
                Volume,Issue=re.search(r", Volume (\d+) Issue (\d+)",issueVolume).groups()
            except:
                Volume,Issue="",""
        except:
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = Art_index.find("a",class_="txt_biaoti")["href"]

                error_variable = "article title"
                Article_title=Art_index.find("a",class_="txt_biaoti").text.strip()

                try:
                    Page_range = Art_index.find("span",class_="abs_njq").contents[1].get_text(strip=True).split()[-1].rstrip(".")
                except:
                    Page_range=""

                try:
                    DOI=Art_index.find("span",class_="abs_njq").find("a",string=lambda text: text and "doi.org/" in text).get_text(strip=True).rsplit("org/",1)[-1]
                except:
                    DOI=""

                error_variable = "pdf link"
                pdf_link=f"https://www.zjujournals.com/agr/EN/article/downloadArticleFile.do?attachType=PDF&id={Art_index["id"].split("art")[-1]}"
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    pdf_headers = {
                        ...
                        #"cookie": "Secure; Hm_lvt_c25a5a1397cbd7c8e9dffe149f6693c9=1726052038,1726052252; HMACCOUNT=89355045786DF1FC; Hm_lpvt_c25a5a1397cbd7c8e9dffe149f6693c9=1726052650; JSESSIONID=75265D9C38DF30D9590140FC68F3F824; Secure; __51cke__=; __tins__19329583=%7B%22sid%22%3A%201726052250618%2C%20%22vd%22%3A%205%2C%20%22expires%22%3A%201726054445332%7D; __51laig__=5",
                        "host": "www.zjujournals.com",
