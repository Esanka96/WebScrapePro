...
while url_index < len(url_list):
    try:
        ...
        url="http://www.gyqx.ac.cn/EN/current"
        ...
        languageList =["http://www.gyqx.ac.cn/EN/current",
                       "http://www.gyqx.ac.cn/CN/current"]

        try:
            volumeYear=currentSoup.find("span",class_="fr").text.strip().split()
            try:
            ...
            try:
                ...
                Year = ""
            try:
                Month=monthCheck(volumeYear[1])
            except:
                print("Failed to find month")
                Month=""
            try:
                Day=dayCheck(volumeYear[0])
            except:
        ...
        All_articles=currentSoup.findAll("ul",class_="lunwen")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_title=All_articles[article_index].find("li",class_="biaoti").a.text.strip()
                Article_link=All_articles[article_index].find("li",class_="biaoti").a.get("href")

                try:
                    Page_range=re.sub(r'\s+'," ",All_articles[article_index].find("span",class_="nianqi").text.strip()).split()[-1].rstrip(".")
                except:
                    print("Failed to find page range")
                    Page_range = ""
                try:
                    DOI=All_articles[article_index].find("span",class_="doi").a.text.strip()
                except:
                    print("Failed to find doi")
                    DOI = ""

                token=get_token(get_soup("http://www.gyqx.ac.cn/CN/home").find("a",class_="current-cover")["href"])

                pdf_link=f"http://www.gyqx.ac.cn/EN/PDF/{DOI}?token={token}"
