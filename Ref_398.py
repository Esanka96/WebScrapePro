...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        baseUrl="https://www.surface-techj.com/bmjsen/ch/"

        currentSoup=get_soup(url)

        languageList =[currentSoup,
                       get_soup("https://www.surface-techj.com/bmjs/ch/index.aspx")]

        try:
            volumeIssue=currentSoup.find("li",class_="nfen").get_text(strip=True)
            pattern=r"(\d{4}),Vol(\d+),No.(\d+)"
            Year,Volume,Issue=re.search(pattern,volumeIssue).groups()
            #print("Slider captcha has been successfully bypassed."+ '\n')
            Day = ""
        except:
            Year,Volume,Issue,Day="","","",""

        All_articles=list(map(lambda x:x.findParent("li"),currentSoup.find("div",class_="right-W").findAll("span",class_="xzk")))
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseUrl+All_articles[article_index].find("dt").a["href"]

                error_variable = "article title"
                Article_title=All_articles[article_index].find("dt").a.get_text(strip=True)

                error_variable = "article details"
                Article_details= get_soup(Article_link)

                try:
                    Page_range = Art_index.find("dt").find_next_sibling("dd").p.get_text(strip=True).split(":")[-1]
                except:
                    Page_range=""

                try:
                    DOI=Article_details.find("span",attrs={"id":"DOI"}).a.get_text(strip=True)
                except:
                    DOI=""

                try:
                    value1,monthValue,value2=re.search(r"(\d{4})(\d{2})(\d{2})",Article_details.find("meta",attrs={"name":"citation_date"})["content"]).groups()
                    Month=datetime.strptime(monthValue,"%m").strftime("%B")
                except:
                    Month=""

                error_variable = "pdf link"
                pdf_link=baseUrl+Art_index.find("a",string=lambda x:x and "PDF" in x)["href"]
