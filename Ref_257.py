...
while url_index < len(url_list):
    try:
        ...
        languageList =["http://www.cjeecmp.cn/EN/2095-9524/current.shtml",
                       "http://www.cjeecmp.cn/CN/2095-9524/current.shtml"]

        currentSoup=get_soup(url)

        try:
            issueVolume = currentSoup.find("h3",class_="prev-next").findParent("div").find("h3").text.strip().split()
            monthYear= currentSoup.find("h3",class_="prev-next").findParent("div").find("h3").findNext("h3").text.strip().split(":")[-1].split()
            try:
                ...
                Volume = ""
            try:
                Issue = re.sub(r'[^0-9]+','',issueVolume[-1])
            except:
                print("Failed to find issue")
                Issue = ""

            try:
                Year = yearCheck(re.sub(r'[^0-9]+','',monthYear[-1]))
            except:
                print("Failed to find year")
                Year = ""
            try:
                Month=monthCheck(monthYear[-2])
            except:
                print("Failed to find month")
                Month=""
            try:
                Day=dayCheck(monthYear[0])
            except:
        ...
        All_articles=currentSoup.find("form",{"id":"AbstractList"}).findAll("div",class_="border-b")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_title=All_articles[article_index].find("div",class_="title").a.text.strip()
                Article_link=All_articles[article_index].find("div",class_="title").a["href"]
                try:
                    pageRangeDoi = re.sub(r'\s+',' ',All_articles[article_index].find("p",class_="page").text.strip())
                    try:
                    ...
                    print("Failed to find doi page range")
                    Page_range,DOI = "",""

                token = get_token('http://www.cjeecmp.cn/EN/2095-9524/current.shtml')
                pdf_link="http://www.cjeecmp.cn/EN/PDF/{}?token={}".format(DOI,token)
