...
while url_index < len(url_list):
    try:
        ...
        url="http://nldxb.njfu.edu.cn/EN/1000-2006/home.shtml"

        languageList =["http://nldxb.njfu.edu.cn/EN/1000-2006/current.shtml",
                       "http://nldxb.njfu.edu.cn/CN/1000-2006/current.shtml"]
        ...
        try:
            ...
            try:
                ...
                Volume = ""
            try:
                Issue = re.sub(r'[^0-9]+','',volumeYear[2])
            except:
            ...
            try:
                ...
                Year = ""
            try:
                Month=monthCheck(volumeYear[-2])
            except:
                print("Failed to find month")
                Month=""
            try:
                Day=dayCheck(volumeYear[-3].split("：")[-1].strip())
            except:
        ...
        All_articles=currentSoup.findAll("div",class_="uw-on-campus")[1].findAll("div",class_="dqml row")
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_title=All_articles[article_index].find("a",class_="text-title").text.strip()
                Article_link=All_articles[article_index].find("a",class_="text-title").get("href")

                try:
                    pageRangeDoi=re.sub(r'\s+',' ',All_articles[article_index].find("p",class_="doi").text.strip())
                    try:
                ...
                token = get_token("http://nldxb.njfu.edu.cn")

                pdf_link=f"http://nldxb.njfu.edu.cn/EN/PDF/{DOI}?token={token}"
