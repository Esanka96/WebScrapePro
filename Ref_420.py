...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        baseUrl="https://archivestsc.com/"

        currentSoup=get_soup(url)

        All_articles = list(map(lambda link: link.find_previous("td"), currentSoup.findAll("a", class_="ListArticleTitle")))
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseUrl+Art_index.find("a",class_="ListArticleTitle")["href"]

                error_variable = "article title"
                Article_title=Art_index.find("a",class_="ListArticleTitle").get_text(strip=True)

                error_variable = "article details"
                Article_details=get_soup(Article_link)

                try:
                    Issue=Article_details.find("meta",attrs={"name":"citation_issue"})["content"]
                    Volume = Article_details.find("meta", attrs={"name": "citation_volume"})["content"]
                    try:
                        preMonth = Article_details.find("meta", attrs={"name": "citation_publication_date"})["content"].split("/")
                        Year=preMonth[0]
                        Month=datetime.strptime(preMonth[1],"%m").strftime("%B")
                    except:
                        Year,Month="",""
                except:
                ...
                try:
                    Page_range="{}-{}".format(Article_details.find("meta", attrs={"name": "citation_firstpage"})["content"],Article_details.find("meta", attrs={"name": "citation_lastpage"})["content"])
                except:
                    Page_range=""

                try:
                    DOI=Article_details.find("a",href=lambda href:href and "org/10." in href).get_text(strip=True)
                except:
                    DOI=""
                Day=""

                error_variable = "pdf link"
                pdf_link=Art_index.find("a",string=lambda href:href and "PDF" in href)["href"]
