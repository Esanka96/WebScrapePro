...
while url_index < len(url_list):
    try:
        ...
        url="https://www.psychiatria-danubina.com/api/issue"
        ...
        attachment = None
        pdf_count = 1
        baseUrl="https://www.psychiatria-danubina.com/"

        currentDetails=get_soup(url)[0]
        Year=currentDetails["year"]
        Volume=currentDetails["volume"]
        Month,Day="",""

        issues=currentDetails["issues"][::-1]
        lastIssue=next((issue for issue in issues if "no" in issue["id"]),None)

        Issue=lastIssue["title"].split()[-1].strip()

        currentSoup=get_soup(f"https://www.psychiatria-danubina.com/api/issue/{lastIssue['id']}")

        All_articles=currentSoup
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseUrl+Art_index["link"]

                error_variable = "article title"
                Article_title=Art_index["title"]

                try:
                    first_page = Article_link.split("_")[-1].split(".pdf")[0]
                    try:
                        Page_range=f"{first_page}-{int(first_page)+pdfPageCount(Article_link)-1}"
                    except:
                        Page_range=""

                    try:
                        DOI=f"10.24869/psyd.{Year}.{first_page}"
                    except:
                        DOI=""
                except:
                    Page_range,DOI="",""
