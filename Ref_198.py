...
def download_pdf(url, out_path):
    ...
    if index==8:
        ...
        with requests.get(url, proxies=formatted_proxies[index],headers=headers, stream=True) as r:
            r.raise_for_status()
            with open(out_path, 'wb') as f:
                for chunk in r.iter_content(chunk_size=8192):
                    if chunk:
...
while url_index < len(url_list):
    try:
        ...
        url="https://www.chinajan.com/EN/home"

        currentSoup=get_soup(url)
        languageList =[currentSoup,get_soup("https://www.chinajan.com/CN/home")]
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_id=All_articles[article_index].get("id").split("art")[-1]

                try:
                    Page_range=All_articles[article_index].find("span",class_="j-volumn").text.strip().split()[-1].rstrip(".")
                except:
                ...
                token = get_token("http://www.chinajan.com")

                pdf_link=f"https://www.chinajan.com/EN/PDF/{DOI}?token={token}"
