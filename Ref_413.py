...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        baseUrl="https://www.clin-lab-publications.com"

        currentSoup=get_soup(url)

        try:
            preMonth=currentSoup.find("h2",class_="topic").get_text(strip=True)
            Issue,Year=re.search(r"Issue No (\d+)/(\d{4})",preMonth).groups()
        except:
            Year,Issue="",""

        try:
            Volume=currentSoup.find("div",class_="info-block-vol__number").get_text(strip=True)
        except:
            Volume=""
        Month,Day="",""

        All_articles=currentSoup.find("h2",class_="topic").find_next_sibling("table").findAll("tr")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseUrl+Art_index.find("a",class_="icon abstract")["href"]
                ...
                Article_title=Article_details.find("span",class_="caption").get_text(strip=True)

                try:
                    DOI=Art_index.find("span",class_="code").get_text(strip=True)
                except:
                    DOI=""
                ...
                pdf_link=f"https://www.clin-lab-publications.com/article/download/{Article_id}"
