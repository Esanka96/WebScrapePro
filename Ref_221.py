...
while url_index < len(url_list):
    try:
        ...
        url="http://cjbmb.bjmu.edu.cn/EN/1007-7626/home.shtml"
        ...
        try:
            volumeYear=currentSoup.find("div",class_="n-j-q").text.strip().split()
            try:
                Volume = re.sub(r'[^0-9]+','',volumeYear[-3])
            except:
            ...
            try:
                ...
                Issue = ""
            try:
                Year = yearCheck(re.sub(r'[^0-9]+','',volumeYear[2]))
            except:
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link=All_articles[article_index].find("div",class_="j-title-1").a.get("href")


                try:
                    Page_range=re.sub(r'\s+'," ",All_articles[article_index].find("span",class_="j-volumn").text.strip()).split()[-1].rstrip(".")
                except:
                    print("Failed to find page range")
                    Page_range = ""
                try:
                    DOI=All_articles[article_index].find("a",class_="j-doi").text.strip().rsplit("org/",1)[-1]
                except:
                    print("Failed to find doi")
                    DOI = ""

                token=get_token("http://cjbmb.bjmu.edu.cn/CN/1007-7626/home.shtml")

                pdf_link="http://cjbmb.bjmu.edu.cn/EN/PDF/{0}?token={1}".format(DOI,token)
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    data.append(
                        {"Title": Article_title, "DOI": DOI, "Publisher Item Type": "", "ItemID": "",
                         ...
                         "Year": Year,
                         "URL": Article_link, "SOURCE File Name": f"{pdf_count}.pdf", "user_id": user_id,"TOC file Name":"TOC_PDF"})
        ...
        while check < 5:
            try:
                ...
                TOC_PDF.get_toc_pdf(current_out)
                check=5
