...
while url_index < len(url_list):
    try:
        ...
        languageList = ["http://www.bcdt.ac.cn/EN/current",
                        "http://www.bcdt.ac.cn/CN/current"]
        currentSoup=get_soup(url)

        try:
            volumeYear = currentSoup.find("h3",class_="latest-issue").text.strip().split()
            try:
                Volume = volumeYear[-3]
            except:
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                try:
                    pageRangeDoi = All_articles[article_index].find("div", class_="j-volumn-doi")
                    try:
                        Page_range = pageRangeDoi.find("span",class_="j-volumn").text.strip().split()[-1].strip().rstrip(".")
                    except:
                        print("Failed to find page range")
                        Page_range = ""
                    try:
                        DOI = pageRangeDoi.find("a",class_="j-doi").text.strip().rsplit("org/",1)[-1]
                    except:
                ...
                token = get_token('http://www.bcdt.ac.cn/EN/current')
                pdf_link=f"http://www.bcdt.ac.cn/EN/PDF/{DOI}?token={token}"
