...
def translate_text(text, source_language='en', target_language='zh-CN'):
...
def get_soup_with_json(url,headers):
    global statusCode
    response = requests.get(url,headers=headers,stream=True)
    statusCode = response.status_code
    return json.loads(BeautifulSoup(response.content, 'html.parser').text)
...
while url_index < len(url_list):
    try:
        ...
        url1="https://www.actasc.cn/rc-pub/front/front-period/getPeriodTree?siteId=302&isBack=false&timestamps="
        ...
        headers1 = {
            ...
            #"Cookie": "acw_tc=b65cfd3317242997927616454e2d370a200814594c44950483ee13446fe7df; language=en; _pk_testcookie..undefined=1; _pk_ses.1.ee79=1; _pk_id.1.ee79=f03442986b657a4e.1724208810.3.1724300016.1724299905.",
            "Host": "www.actasc.cn",
            "Language": "en",
            "Pragma": "no-cache",
        ...
        currentSoup=get_soup_with_json(f"https://www.actasc.cn/rc-pub/front/front-article/getArticlesByPeriodicalIdGroupByColumn/{get_soup_with_json(url1,headers1)["data"][0]["periods"][0]["periods"][0]["id"]}?showCover=false&timestamps=",headers2)
        ...
        first_line=f"Vol. {inX["volume"]}， Issue {inX["issue"]}， {inX["year"]}"
        html_body_en += f"<h2>{first_line}</h2>\n"

        first_line_cn=f"{inX["year"]}年第{inX["volume"]}卷第{inX["issue"]}期"
        ...
        while article_index < len(All_articles):
            ...
            error_variable = None
            try:
                index=All_articles[article_index]

                error_variable = "article metadata"
                Article_title=index["enResName"]
                Volume = index["volume"]
                Issue = index["issue"]
                Year = index["year"]
                DOI = index["doi"]
                Page_range = f"{index["firstPageNum"]}-{index["lastPageNum"]}"
                Article_link = f"https://www.actasc.cn/thesisDetails#{DOI}&lang=en"
                pdf_link=f"https://www.actasc.cn/rc-pub/front/front-article/download?id={index["id"]}&attachType=lowqualitypdf&token=&language=en"
                Month,Day="",""

                enAuthors = index["enAuthors"]
                cnAuthors= index["authors"]

                enSummary = index["enSummary"]
                cnSummary = index["summary"]

                enKeyword = index["enKeyword"]
                cnKeyword = index["keyword"]

                enPublic = index["publishDate"].split()[0]
                ...
                html_body_en += f"<p>{enAuthors}</p>\n"
                html_body_en += f"<p>Vol. {Volume}, Issue {Issue}, Pages: {Page_range}({Year}) DOI: {DOI}</p>\n"
                html_body_en += f"<p>{enSummary}</p>\n"
                html_body_en += f"<p>Keywords: {enKeyword}</p>\n"
                html_body_en += f"<p>Updated：{enPublic}</p>\n"
                html_body_en += "</div>\n"

                html_body_cn += f"<div class='article'>\n"
                html_body_cn += f"<a href=href='{Article_link}' style='text-decoration:none;' color:#00f;'><h3 >{translate_text(index["resName"])}</h3></a>\n"
                html_body_cn += f"<p>{translate_text(cnAuthors)}</p>\n"
                html_body_cn += f"<p>{Year}, {Volume}({Issue}): {Page_range}. DOI: {DOI}</p>\n"
                html_body_cn += f"<p>{translate_text(cnSummary)}</p>\n"
                html_body_cn += f"<p>关键词: {translate_text(cnKeyword)}</p>\n"
                html_body_cn += f"<p>更新时间：{enPublic}</p>\n"
                html_body_cn += "</div>\n"
                ...
                if Check_duplicate.lower() == "true" and check_value:
                    ...
                    download_pdf(pdf_link, output_fimeName,headers)
