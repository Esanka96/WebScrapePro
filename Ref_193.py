...
while url_index < len(url_list):
    try:
        ...
        url="https://hgjz.cip.com.cn/EN/1000-6613/current.shtml"
        ...
        languageList =["https://hgjz.cip.com.cn/EN/1000-6613/current.shtml",
                       "https://hgjz.cip.com.cn/CN/1000-6613/current.shtml"]

        currentSoup=get_soup(url)

        try:
            volumeYear=Year_volume=currentSoup.find("div",class_="njq").text.strip().split()
            ...
            try:
                Year = re.sub(r'[^0-9]+','',volumeYear[-5])
            except:
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                try:
                    pageRangeDoi = All_articles[article_index].find("dd",class_="kmnjq")
                    try:
                        pageRange=re.sub(r'\s+',' ',pageRangeDoi .text.strip())
                        Page_range=re.search(r'\): (.*?). doi', pageRange).group(1)
                    except:
                        print("Failed to find page range")
                        Page_range = ""

                    try:
                        DOI=pageRangeDoi.find("a").text.strip()
                    except:
                ...
                token = get_token('https://hgjz.cip.com.cn/CN/1000-6613/current.shtml')

                pdf_link=f"https://hgjz.cip.com.cn/EN/PDF/{DOI}?token={token}"
