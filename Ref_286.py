...
while url_index < len(url_list):
    try:
        ...
        currentSoup=get_soup(baseUrl+get_soup(baseUrl).find("a",string="CURRENT ISSUE")["href"])

        try:
            volumeIssue=currentSoup.find("div",class_="highwire-cite-metadata").text.strip()
            pattern=r"(\w+) (\d+); Volume (\d+),Issue ([A-Za-z0-9]+)"
            Month,Year,Volume,Issue=re.search(pattern,volumeIssue).groups()
        except:
            Year,Volume,Issue,Month="","","",""
        Day=""

        All_articles=currentSoup.findAll("div",class_="toc-citation")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseUrl+All_articles[article_index].find("a",class_="highwire-cite-linked-title")["href"]

                error_variable = "article title"
                Article_title=All_articles[article_index].find("span",class_="highwire-cite-title").text.strip()

                try:
                    DOI=All_articles[article_index].find("span",class_="highwire-cite-metadata-doi").text.strip().rsplit("org/",1)[-1]
                except:
                    DOI=""

                try:
                    Page_range=All_articles[article_index].find("span",class_="highwire-cite-metadata-pages").text.strip().rstrip(".")
                except:
                    Page_range=""

                error_variable = "pdf link"
                pdf_link=baseUrl+get_soup(Article_link).find("a",attrs={"data-panel-name":"jnl_bjgp_tab_pdf"})["href"]
