...
while url_index < len(url_list):
    try:
        ...
        attachment = None
        pdf_count = 1
        baseUrl = "http://www.zhilengxuebao.com/zlxben/ch/"

        currentSoup=get_soup(url)

        languageList =[currentSoup,
                       get_soup("http://www.zhilengxuebao.com/zlxb/ch/index.aspx")]

        try:
            volumeIssue=currentSoup.find("div",class_="njq_cbrq").find("div",class_="le").text.strip().split()
            Volume=volumeIssue[1]
            Issue=volumeIssue[2]
            Year=yearCheck(volumeIssue[0])
        except:
            Year,Volume,Issue="","",""
        Day,Month="",""

        All_articles=currentSoup.find("div",attrs={"id":"wrap"}).find("ul",class_="Article_list").findAll("li")
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                Article_link = baseUrl+All_articles[article_index].find("dd",class_="wzti").a["href"]

                error_variable = "article title"
                Article_title=All_articles[article_index].find("dd",class_="wzti").a.text.strip()

                try:
                    DOI=All_articles[article_index].find('div', class_='Abs').findAll("a")[-1].text.strip().rsplit("DOI:",1)[-1]
                except:
                    DOI=""

                try:
                    Page_range = All_articles[article_index].find('div', class_='Abs').find("font").next_sibling.string.split("):")[-1].split()[0]
                except:
                    Page_range=""

                error_variable = "pdf link"
                pdf_link=baseUrl+All_articles[article_index].find('div', class_='Abs').find('a', href=lambda href: href and 'create_pdf' in href)["href"]
