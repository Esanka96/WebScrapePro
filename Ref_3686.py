...
while url_index < len(url_list):
    try:
        ...
        try:
            preMonth=currentSoup.find("div",class_="page page_issue").h1.get_text(strip=True)
            Year,Volume,Issue=re.search(r"(\d{4}),Vol. (\d+), No.(\d+)",preMonth).groups()
        except:
            Year,Volume,Issue="","",""
        Month,Day="",""
        ...
        while article_index < len(All_articles):
            ...
            try:
                ...
                try:
                    Page_range=Art_index.find("div",class_="pages").get_text(strip=True).split("pp.")[-1]
                except:
                    Page_range=""
