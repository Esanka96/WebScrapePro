...
while url_index < len(url_list):
    try:
        ...
        languageList =["https://xuebaozk.haut.edu.cn/en/article/current",
                       "https://xuebaozk.haut.edu.cn/cn/article/current"]

        currentSoup=get_soup("https://xuebaozk.haut.edu.cn"+get_soup(url).find("li",{"type":"currentIssue_en"}).a["href"])
        ...
        while article_index < len(All_articles):
            ...
            try:
                Article_title=All_articles[article_index].find("div",class_="article-list-title").a.text.strip()
                Article_link="https://xuebaozk.haut.edu.cn"+All_articles[article_index].find("div",class_="article-list-title").a.get("href")

                try:
                    volumeYear=All_articles[article_index].find("div",class_="article-list-time").font.text.strip().split(":")[0].split()

                    try:
                        Volume = re.sub(r'[^0-9]+','',volumeYear[1].split("(")[0])
                    except:
                        print("Failed to find volume")
                        Volume = ""
                    try:
                        Issue = re.sub(r'[^0-9]+','',volumeYear[1].split("(")[1])
                    except:
                        print("Failed to find issue")
                        Issue = ""
                    try:
                        Year = yearCheck(re.sub(r'[^0-9]+','',volumeYear[0]))
                    except:
                ...
                try:
                    Page_range=All_articles[article_index].find("div",class_="article-list-time").font.text.strip().split(":")[1].rstrip(".")
                except:
                    print("Failed to find page range")
                    Page_range = ""
                try:
                    DOI=All_articles[article_index].find("div",class_="article-list-time").a.text.strip()
                except:
                    print("Failed to find doi")
                    DOI = ""

                ArticleId=getPdfId(All_articles[article_index].find("div",class_="article-list-zy article-list-btn clear").find("font",class_="font3").find("a").get("onclick"))
                pdf_link=f"https://xuebaozk.haut.edu.cn/article/exportPdf?id={ArticleId}&language=en"
